            ...
            users.forEach((doc) => {
                if (doc.data().email === this.state.email) {
                    ...
                    if (doc.data().password === this.state.password) {
                        Toast.show({type: 'success', text1: "Welcome back !", visibilityTime: 2000});
                        this.context.setIsAuth(doc.id);
                        return
                    } else {

                        Toast.show({type: 'error', text1: "Wrong password...", visibilityTime: 2000});
                        return
                    }
                }
            });
            if (! exist) {
                Toast.show({type: 'error', text1: "User doesn't exist...", visibilityTime: 2000});
            }
        });
                ...
                <View style={
                    ...
                    <View>
                        ...
                        <Text style={
                            ...
                            onPress=
                            {() => this.props.navigation.navigate('Register')}>Don't have an account ? Register !</Text>
